...
| `merge` | not-supported | pyFF's `merge` action merges results from a `fork` branch back into the outer pipeline state; goFF's update-pipeline model uses fork branches for side-effect publishing only — branch results are intentionally discarded and `merge` will be rejected as an unknown action |
...
| MDQ server backed by pipeline-built repository | supported | Startup load + periodic refresh loop implemented via pipeline execution; `/entities/{sha1}HEXHASH` pyFF MDQ SHA1-form lookup is supported (SHA-1 of the entity ID bytes expressed as lowercase hex); `ETag` header is set on XML entity responses (SHA-256[:8] of body as quoted hex string); `If-None-Match` matching returns 304 with empty body |
